...
class SignUpView(View):
    def post(self, request):
        ...
        if ('email' or 'phone_number') not in data.keys()\
            ...
            return JsonResponse({'message':'INVALIDE'}, status=400)

        #중복가입 방지
        elif (
            ('phone_number' in data.keys()\
            and {'phone_number': data['phone_number']} in list(Users.objects.values('phone_number')))\
            or ('email' in data.keys()\
            and {'email': data['email']} in list(Users.objects.values('email')))\
            or ('user_name' in data.keys()\
            and {'user_name': data['user_name']} in list(Users.objects.values('user_name')))\
            ):
