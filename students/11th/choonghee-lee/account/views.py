...
class UserRegisterView(View):
    ...
    def post(self, request, **kwargs):
        ...
        if result_code == ERROR_INVALID_INPUT:
            ...
            has_same_phone_or_email=User.objects.filter(phone_or_email=phone_or_email)
            has_same_username = User.objects.filter(username=username)

            if has_same_phone_or_email or has_same_username:
                return JsonResponse({'message': 'ALREADY_SIGNED_UP'}, status=409)
            else:
                bcrypted_password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
                User(
