...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 이메일 형식 지키지 않았을 경우
            if (email_analysis.match(data['email']) !=None) == False :
                return JsonResponse({"message" : "EMAIL_ERROR"}, status=400)  

            # 중복된 row값이 들어갈 경우        
            if User.objects.filter(name = data['name']) or User.objects.filter(email = data['email'])\
                                                    or User.objects.filter(telephone = data['telephone']) :
                return JsonResponse({"message" : "OVERLAP_ERROR"}, status=400)    

            User(name=data['name'],telephone =data['telephone'], password=data['password'], email = data['email']).save()            
